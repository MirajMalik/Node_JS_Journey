...
/*Custom Middleware Creation.After completing middleware function works we need to
call next function(err).Otherwise the browser will continue loading
*/

// function customMiddleware(req,res,next){
//     // console.log('I am logged in.')
//     console.log(req.url) // we can use req and res object here for our work.
//     next()
// }

//if we want to block a page when routes to /posts

function customMiddleware(req,res,next){
    // by using this middleware,now the handler of /posts wont work.
    if(req.url==='/posts'){
        res.send('<h1>Sorry,This page is blocked by Admin.</h1>')
    }
    // for all the routes do as it was.
    next()
}
...
// to use this custom middleware to all routes we use app.use(funnction name)
